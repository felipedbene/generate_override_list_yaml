...
```bash
csv-to-yaml$ sam build --use-container
```

The SAM CLI installs dependencies defined in `transform/requirements.txt`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
csv-to-yaml$ sam local invoke TransformFunction --event events/event.json
```



```bash
csv-to-yaml$ sam logs -n HelloWorldFunction --stack-name csv-to-yaml --tail
```
...
```bash
csv-to-yaml$ pip install -r tests/requirements.txt --user
# unit test
csv-to-yaml$ python -m pytest tests/unit -v
...
```bash
aws cloudformation delete-stack --stack-name csv-to-yaml
```
